...
- Approved songs download and convert **one at a time** in a separate Python process. yt-dlp searches YouTube; FFmpeg normalizes to stereo 44.1 kHz / 128 kbps MP3 with the bit reservoir disabled. This permits new listeners to decode from any complete frame. Files become visible atomically after validation. Failures retry after an hour and are logged; details are also stored in SQLite's `media` table.
