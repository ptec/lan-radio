...
(async()=>{
 ...
 assert.equal(context.document.title,'Staff Radio');
 ...
 assert.equal(context.document.title,'Rock song — Band | Staff Radio','Title follows listening, not browsing');
 ...
 stations[1].now_playing={title:'Next track',artist:'New artist'};
 await vm.runInContext('refresh()',context);
 assert.equal(context.document.title,'Next track — New artist | Staff Radio');
